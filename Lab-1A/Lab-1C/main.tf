############################################
# Locals (naming convention: Bentley-*)
############################################
locals {
...
# Explanation: Bentley needs a hyperlane—this VPC is the Millennium Falcon’s flight corridor.
resource "aws_vpc" "bentley_vpc01" {
...
resource "aws_internet_gateway" "bentley_igw01" {
...
resource "aws_subnet" "bentley_public_subnets" {
...
resource "aws_subnet" "bentley_private_subnets" {
...
# Explanation: Bentley wants the private base to call home—EIP gives the NAT a stable “holonet address.”
resource "aws_eip" "bentley_nat_eip01" {
  domain = "vpc"
...
# Explanation: NAT is bentley’s smuggler tunnel—private subnets can reach out without being seen.
resource "aws_nat_gateway" "bentley_nat01" {
  allocation_id = aws_eip.bentley_nat_eip01.id
  subnet_id     = aws_subnet.bentley_public_subnets[0].id # NAT in a public subnet

  tags = {
    Name = "${local.name_prefix}-nat01"
  }

  depends_on = [aws_internet_gateway.bentley_igw01]
}
...
resource "aws_route_table" "bentley_public_rt01" {
  vpc_id = aws_vpc.bentley_vpc01.id
...
resource "aws_route" "bentley_public_default_route" {
  route_table_id         = aws_route_table.bentley_public_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.bentley_igw01.id
}

# Explanation: Attach public subnets to the “public lanes.”
resource "aws_route_table_association" "bentley_public_rta" {
  count          = length(aws_subnet.bentley_public_subnets)
  subnet_id      = aws_subnet.bentley_public_subnets[count.index].id
  route_table_id = aws_route_table.bentley_public_rt01.id
}

# Explanation: Private route table = “stay hidden, but still ship supplies.”
resource "aws_route_table" "bentley_private_rt01" {
  vpc_id = aws_vpc.bentley_vpc01.id
...
# Explanation: Private subnets route outbound internet via NAT (Bentley-approved stealth).
resource "aws_route" "bentley_private_default_route" {
  route_table_id         = aws_route_table.bentley_private_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = aws_nat_gateway.bentley_nat01.id
}

# Explanation: Attach private subnets to the “stealth lanes.”
resource "aws_route_table_association" "bentley_private_rta" {
  count          = length(aws_subnet.bentley_private_subnets)
  subnet_id      = aws_subnet.bentley_private_subnets[count.index].id
  route_table_id = aws_route_table.bentley_private_rt01.id
}
...
# Explanation: EC2 SG is bentley’s bodyguard—only let in what you mean to.
resource "aws_security_group" "bentley_ec2_sg01" {
...
resource "aws_security_group" "bentley_rds_sg01" {
...
resource "aws_vpc_security_group_ingress_rule" "rds_mysql_from_ec2" {
  security_group_id            = aws_security_group.bentley_rds_sg01.id
  referenced_security_group_id = aws_security_group.bentley_ec2_sg01.id
  from_port                    = 3306
  to_port                      = 3306
  ip_protocol                  = "tcp"
...
resource "aws_db_subnet_group" "bentley_rds_subnet_group01" {
...
resource "aws_db_instance" "bentley_rds01" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.bentley_rds_subnet_group01.name
  vpc_security_group_ids = [aws_security_group.bentley_rds_sg01.id]
...
# Explanation: Bentley refuses to carry static keys—this role lets EC2 assume permissions safely.
resource "aws_iam_role" "bentley_ec2_role01" {
...
resource "aws_iam_role_policy_attachment" "bentley_ec2_ssm_attach" {
...
resource "aws_iam_role_policy_attachment" "bentley_ec2_secrets_attach" {
  role       = aws_iam_role.bentley_ec2_role01.name
...
resource "aws_iam_role_policy_attachment" "bentley_ec2_cw_attach" {
  role       = aws_iam_role.bentley_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}

# Explanation: Instance profile is the harness that straps the role onto the EC2 like bandolier ammo.
resource "aws_iam_instance_profile" "bentley_instance_profile01" {
...
resource "aws_instance" "bentley_ec201" {
  ami                    = var.ec2_ami_id
  instance_type          = var.ec2_instance_type
  subnet_id              = aws_subnet.bentley_public_subnets[0].id
  vpc_security_group_ids = [aws_security_group.bentley_ec2_sg01.id]
  ...
  # depends_on = [aws_db_instance.bentley_rds01]
...
resource "aws_instance" "bentley_ec2_01" {
  ami                         = var.ec2_ami_id
  instance_type               = var.ec2_instance_type
  subnet_id                   = aws_subnet.bentley_public_subnets[0].id
  vpc_security_group_ids      = [aws_security_group.bentley_ec2_sg01.id]
  ...
  tags = {
    Name = "${local.name_prefix}-ec2_02-public"
  }
}

resource "aws_instance" "bentley_ec_03" {
  ami                         = var.ec2_ami_id
  instance_type               = var.ec2_instance_type
...
# Explanation: Parameter Store is bentley’s map—endpoints and config live here for fast recovery.
resource "aws_ssm_parameter" "bentley_db_endpoint_param" {
...
resource "aws_ssm_parameter" "bentley_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
  value = tostring(aws_db_instance.bentley_rds01.port)
...
resource "aws_ssm_parameter" "bentley_db_name_param" {
  name  = "/lab/db/name"
  type  = "String"
  value = var.db_name
...
# Explanation: Secrets Manager is bentley’s locked holster—credentials go here, not in code.
resource "aws_secretsmanager_secret" "bentley_db_secret01" {
  name                    = "aws-armageddon-7/rds/mysql1"
  recovery_window_in_days = 0
}

# Explanation: Secret payload—students should align this structure with their app (and support rotation later).
resource "aws_secretsmanager_secret_version" "bentley_db_secret_version01" {
  secret_id = aws_secretsmanager_secret.bentley_db_secret01.id

  secret_string = jsonencode({
    username = var.db_username
    password = var.db_password
    host     = aws_db_instance.bentley_rds01.address
    port     = aws_db_instance.bentley_rds01.port
    dbname   = var.db_name
  })
}
...
resource "aws_cloudwatch_log_group" "bentley_log_group01" {
...
# Explanation: Metrics are bentley’s growls—when they spike, something is wrong.
# NOTE: Students must emit the metric from app/agent; this just declares the alarm.
resource "aws_cloudwatch_metric_alarm" "bentley_db_alarm01" {
  ...
  alarm_actions = [aws_sns_topic.bentley_sns_topic01.arn]
...
resource "aws_sns_topic" "bentley_sns_topic01" {
...
resource "aws_sns_topic_subscription" "bentley_sns_sub01" {
  topic_arn = aws_sns_topic.bentley_sns_topic01.arn
  protocol  = "email"
...
# resource "aws_vpc_endpoint" "bentley_vpce_ssm" { ... }